...
@app.route('/data/basic/realeco/name', methods=['GET'])
def getRealecoName():
    data = []
    df = pd.read_excel(os.path.join(Settings.data_url, 'basic', 'basic.xls'))
    for col in df.columns.values:
        data.append({'value': col, 'label': col})
    return jsonify({'data': data, 'Category': '实体经济'})


@app.route('/data/basic/realeco/single/<index_name>', methods=['GET'])
