...
@app.route('/data/macro/credit/name', methods=['GET'])
def getCreditName():
    data = []
    df = pd.read_excel(os.path.join(Settings.data_url, 'macro', 'credit.xls'))
    for col in df.columns.values:
        data.append({'value': col, 'label': col})
    return jsonify({'data': data, 'Category': '国内宏观', 'SubCategory': '信贷'})


@app.route('/data/macro/credit/single/<name>', methods=['GET'])
def getSinglecredit(name):
