...
@app.route('/data/industry/metal/name', methods=['GET'])
def getMetalName():
    data = []
    df = pd.read_excel(os.path.join(Settings.data_url, 'industry', 'metal.xls'))
    for col in df.columns.values:
        data.append({'value': col, 'label': col})
    return jsonify({'data': data, 'Category': '基本金属'})


@app.route('/data/industry/metal/single/<index_name>', methods=['GET'])
def getSingleMetal(index_name):
