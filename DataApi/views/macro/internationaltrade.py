...
@app.route('/data/macro/internationaltrade/name', methods=['GET'])
def getInternationaltradeName():
    data = []
    df = pd.read_excel(os.path.join(Settings.data_url, 'macro', 'internationaltrade.xls'))
    for col in df.columns.values:
        data.append({'value': col, 'label': col})
    return jsonify({'data': data, 'Category': '国内宏观', 'SubCategory': '国际贸易'})


@app.route('/data/macro/internationaltrade/single/<name>', methods=['GET'])
def getSingleInternationaltrade(name):
